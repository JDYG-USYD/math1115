...
- Box model
- Use the Binomial Formula to model chance

A binary trial is where only two things can occur, so, *P*(event) = *p* and *P*(non-event) = 1 - *p*. 
...
When drawing (a reasonably large number) at random with replacement from a box, the probability histogram (in standard units) for the sum (or average) will follow the normal curve. 
...
# mean and SD of box
mean(box)
popsd(box) #how is popsd() different from sd()? 
```
