...
> **Research question**: Is there a linear relationship between sprint time and the year of the race, for male record holders?

We focus on male record holders and create a linear model between `male` individuals and `year`. *Note: steps are outlined for demonstration purposes.*

## **Step 1:** generate the scatter plot {-}

If we want, we could always use `geom_smooth()` to explore the "shape" of the data. 
